...
def \
        prepare_kb_envrioment(raw_kb_path, train_path, dev_path, test_path, test_mode, add_reverse_relations=True):
    """
    ...
    tailentity_dict = collections.defaultdict(int)
    action_num = []
    for e1 in adj_list:
        for r in adj_list[e1]:
            action_dict[e1] += len(adj_list[e1][r])
            key = int(len(adj_list[e1][r])/10) + 1
            if not key in tailentity_dict:
                tailentity_dict[key] = 0
            tailentity_dict[key] += 1
        action_num.append((e1, action_dict[e1]))

    action_num.sort(key=lambda x: x[1])

    actions_num_path = os.path.join(data_dir, 'actions_num.txt')
    tailentity_num_path = os.path.join(data_dir, 'tailentity_num.txt')
    with open(actions_num_path, 'w') as f:
        for n in action_num:
            f.write(str(n[0]) + " " + str(n[1]) + "\n")
    with open(tailentity_num_path, 'w') as f:
        for key in sorted(tailentity_dict.keys()):
            f.write(str(key)+" "+str(tailentity_dict[key])+"\n")
    # Save adjacency list
