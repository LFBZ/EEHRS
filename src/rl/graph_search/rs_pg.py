...
class RewardShapingPolicyGradient(PolicyGradient):
    def __init__(self, args, kg, pn, fn_kg, fn, fn_secondary_kg=None):
        ...
        self.set_high_reward = args.set_high_reward
    ...
    def reward_fun(self, e1, r, e2, pred_e2, relation_selected=[]):
        if self.model.endswith('.rso'):
            ...
            if self.fn_secondary_kg:
                ...
                if self.set_high_reward:
                    R_list = [self.fn_kg.get_relation_embeddings(relation_selected[i]) for i in range(1, self.num_rollout_steps+1)]
                    Q = self.fn_kg.get_relation_embeddings(r)
                    Rs = torch.stack(R_list, dim=0)
                    R = torch.sum(Rs, dim=0)
                    high_reward = F.relu(F.cosine_similarity(Q, R, dim=-1))
                else:
                    high_reward = real_reward
            ...
            if self.model.endswith('rsc'):
                ...
                if self.rl_module == 'original':
                    ...
                    reward_high = binary_reward + self.mu * (1 - binary_reward) * high_reward
